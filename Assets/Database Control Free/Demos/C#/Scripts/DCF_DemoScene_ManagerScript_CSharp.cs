...
public class DCF_DemoScene_ManagerScript_CSharp : MonoBehaviour {
    ...
    IEnumerator LoginUser ()
    {
        ...
        if (response == "Success")
        {
            ...
            LoggedIn_DisplayUsernameText.text = "เข้าสู่ระบบด้วยชื่อ : " + playerUsername;
        } else
        {
    ...
    {
        IEnumerator e = DCF.RegisterUser(playerUsername, playerPassword, "ไม่มีประวัติการเล่น"); // << Send request to register a new user, providing submitted username and password. It also provides an initial value for the data string on the account, which is "Hello World".
        while (e.MoveNext())
        {
            yield return e.Current;
        }
        ...
        } else
        {
            ...
            if (response == "UserError")
            {
                ...
                Register_ErrorText.text = "Error: Username นี้มีในระบบแล้ว";
            } else
            {
                //There was another error. This error message should never appear, but is here just in case.
                Login_ErrorText.text = "Error: Unknown Error. กรุณาลองใหม่อีกครั้ง";
    ...
    public void Login_LoginButtonPressed ()
    {
        ...
        } else
        {
            ...
            Login_ErrorText.text = "Error: Username ไม่ถูกต้อง";
        }
    }
        ...
        if (playerUsername.Length > 3)
        {
            if (playerPassword.Length > 5)
            {
                ...
                else
                {
                    ...
                    Register_ErrorText.text = "Error: รหัสไม่ถูกต้อง";
                }
            }
            else
            {
                //Password too short so show error
                Register_ErrorText.text = "Error: รหัสสั้นเกินไป";
            }
        }
        else
        {
            //Username too short so show error
            Register_ErrorText.text = "Error: Username สั้นเกินไป";
        }
    }
